...
| Low-mode subspace/action rather than Hessian component MAE controls ionic response | Plausible and already diagnosed, not yet causal | The then-proposed response-operator action objective subsequently failed its 32-material capacity gate and was removed from the maintained trainer | Its immutable output remains evidence; do not reintroduce it as a fallback. |
