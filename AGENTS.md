...
- The production ionic path is the differentiable factor/Schur response
  `Z*^T D_delta(Phi)Lambda`, where `D_delta` is the real Tikhonov resolvent
  with spectral filter `lambda/(lambda^2+delta^2)`. It is a bounded response
  coordinate on soft/unstable references, not an unregularized equilibrium
  derivative. The independently predicted translation-free `U_eta,delta`
  head is retained as an amortized-solver diagnostic and is not the production
  ionic path.
- `Phi/Lambda` propagation is an attached physical path. Strict labels
...
- Total-only GMTNet labels use an independent macro encoder/head. They must not
  ...
  `Lambda`, or `U_eta`.
- Do not add a second constitutive-density wrapper around the independent
  direct-`U` diagnostic. The production factor/Schur branch already has the
  declared scalar-factor-energy semantics; report the direct-`U` agreement and
  strict first-order U/V residual as diagnostics.
...
- `model_from_config` accepts only `independent_quadratic_response`, isotropic
  background, and the continuous `tikhonov` operator (with `regularized` kept
  only as a compatibility alias). Exact propagation is an explicit true-DFPT
  stable diagnostic. There is no `auto` policy.
